...
from proj1 import models as proj1_models
from . import models
import time
...
def sessioncheckuser_middleware(get_response):
    def middleware(request):
        if request.path=='/user/' or request.path=='/user/funds/' or request.path=='/user/success/':
...
def cpuser(request):
 if request.method=="GET":   
  return render(request,"cpuser.html",{"sunm":request.session["sunm"]})
    ...
    userDetails=proj1_models.Register.objects.filter(email=sunm,password=opassword)
  
    if len(userDetails)>0:
      if npassword==cnpassword:
       proj1_models.Register.objects.filter(email=sunm).update(password=cnpassword)
...
def epuser(request): 
 if request.method=="GET":
  userDetails=proj1_models.Register.objects.filter(email=request.session["sunm"])
  ...
  proj1_models.Register.objects.filter(email=request.session["sunm"]).update(name=name,mobile=mobile,address=address,city=city,gender=gender)
  return redirect("/user/epuser/")

def funds(request): 
 paypalURL="https://www.sandbox.paypal.com/cgi-bin/webscr"
 paypalID="sb-y47yla26639395@business.example.com"
 amt=100
 #sb-wa4rj26997750@personal.example.com
