...
def manageusers(request): 
 userDetails=proj1_models.Register.objects.filter(role="user")
...
def manageuserstatus(request):
 ...
 if status==1:
  proj1_models.Register.objects.filter(regid=regid).update(status=status)
 elif status==0:
  proj1_models.Register.objects.filter(regid=regid).update(status=status)
 else:
  proj1_models.Register.objects.filter(regid=regid).delete()
